...
/* Legacy Prime commands are request/response exchanges.  Discard reports
 * which were already queued before a new request is sent; otherwise a delayed
 * control report from the preceding exchange can be mistaken for report zero
 * of the new virtual packet.  Keep this separate from sendReport() so a reply
 * delivered while sendReport() resolves is never discarded. */
EM_JS(void, prime_webhid_begin_legacy_exchange_js, (), {
    const state = Module.__hplpWebHID;
    if (state) {
        state.queue = [];
    }
